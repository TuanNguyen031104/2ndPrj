...
import { GetAllMovies, GetMovieById } from "../../apiIntergration/movies";
import { useNavigate, useParams } from "react-router-dom";
import moment from "moment";
import { GetAllTheatresByMovie } from "../../apiIntergration/theatres";
...
function TheatresForMovie() {
    ...
    const getTheatres = async () => {
        try {
            dispatch(ShowLoading());
            console.log("Fetching theatres with:", { date, movie: params.id });
            const response = await GetAllTheatresByMovie({ date, movie: params.id });
            console.log("GetAllTheatresByMovie response:", response);
            if (response.success) {
            ...
            <div>
                ...
                <div className="flex justify-between items-center mb-2">
                    <div>
                        ...
                        <h1 className="text-md">Thời lượng : {movie.duration} phút</h1>
                        <h1 className="text-md">
                            Ngày ra mắt : {moment(movie.releaseDate).format("MMM Do yyyy")}
                        </h1>
                        <h1 className="text-md">Thể loại : {movie.genre}</h1>
                    </div>

                    <div>
                        <h1 className="text-md">Chọn ngày</h1>
                        <input
                            type="date"
